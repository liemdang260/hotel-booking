...
func (h *Handler) Register(ctx context.Context, r Credentials) (RegisteredUser, error) {
	user, err := h.auth.Register(ctx, r.Email, r.Password)
	if err != nil {
...
func (h *Handler) Login(ctx context.Context, r Credentials) (usecase.Tokens, error) {
	return h.auth.Login(ctx, r.Email, r.Password)
}

func (h *Handler) Refresh(ctx context.Context, token string) (usecase.Tokens, error) {
	return h.auth.Refresh(ctx, token)
}

func (h *Handler) Logout(ctx context.Context, token string) error {
	return h.auth.Logout(ctx, token)
}
