...
RETURNING r.payment_id,
 ...
 r.last_error_code,r.version,r.created_at,r.updated_at`, now, limit, leaseUntil)
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&j.PaymentID, &j.IdempotencyKey, &j.ProviderReference, &j.Status,
			&j.RetryCount, &j.MaxAttempts, &j.NextRetryAt, &j.LeaseUntil,
			&j.LastErrorCode, &j.Version, &j.CreatedAt, &j.UpdatedAt,
		); err != nil {
			_ = rows.Close()
			return nil, err
		}
...
func (r *PaymentRepository) Resolve(
	ctx context.Context,
	paymentID string,
	version int64,
	status domain.Status,
	providerRef, failure string,
	now time.Time,
) (domain.Payment, error) {
	if status != domain.StatusSucceeded && status != domain.StatusFailed {
		return domain.Payment{}, domain.ErrInvalidTransition
	}
	tx, err := r.db.BeginTx(ctx, nil)
	if err != nil {
	...
	if n != 1 {
		return domain.Payment{}, repository.ErrConcurrentUpdate
	}
	...
	p, err := scanPayment(tx.QueryRowContext(ctx, "SELECT "+paymentColumns+" FROM payments WHERE id=$1", paymentID))
	if err != nil {
		return domain.Payment{}, err
	}
	if err = tx.Commit(); err != nil {
