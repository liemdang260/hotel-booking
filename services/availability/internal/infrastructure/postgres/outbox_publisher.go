...
func(s *OutboxPublisherStore)Claim(ctx context.Context,request outbox.ClaimRequest)([]outbox.Event,error){
 ...
 SELECT id FROM availability_outbox_events
...
UPDATE availability_outbox_events AS event
...
RETURNING event.id::text,event.aggregate_type,event.aggregate_id::text,event.aggregate_version,event.event_type,
	...
	if err:=rows.Close();err!=nil{return nil,fmt.Errorf("close availability outbox rows: %w",err)}
	if err:=tx.Commit();err!=nil{return nil,fmt.Errorf("commit availability outbox claim: %w",err)}
	return events,nil
}

func(s *OutboxPublisherStore)MarkPublished(ctx context.Context,id,token string,publishedAt time.Time)error{
	result,err:=s.db.ExecContext(ctx,`UPDATE availability_outbox_events
 ...
 WHERE id=$1::uuid AND claim_token=$2::uuid AND status='PUBLISHING'`,id,token,publishedAt)
	return requireAvailabilityOne(result,err,"mark availability outbox published")
}
func(s *OutboxPublisherStore)MarkRetry(ctx context.Context,id,token string,nextAttempt time.Time,message string)error{
	result,err:=s.db.ExecContext(ctx,`UPDATE availability_outbox_events
 SET status='PENDING',attempt_count=attempt_count+1,available_at=$3,claim_token=NULL,claimed_at=NULL,locked_until=NULL,last_error=left($4,1024)
 WHERE id=$1::uuid AND claim_token=$2::uuid AND status='PUBLISHING'`,id,token,nextAttempt,message)
	return requireAvailabilityOne(result,err,"mark availability outbox retry")
}
func requireAvailabilityOne(result sql.Result,err error,operation string)error{
