...
var (
	...
	ErrQuoteMismatch = errors.New("quote does not match booking request")
	ErrSoldOut = errors.New("inventory sold out")
	ErrInventoryNotConfigured = errors.New("inventory not configured")
	ErrReservationNotFound = errors.New("reservation not found")
	ErrReservationExpired = errors.New("reservation expired")
	ErrIdempotencyConflict = errors.New("idempotency conflict")
	ErrPaymentDeclined = errors.New("payment declined")
	ErrPaymentNotFound = errors.New("payment not found")
	ErrOutcomeUnknown = errors.New("remote mutation outcome unknown")
...
const (
	...
	ReservationExpiredStatus ReservationStatus = "EXPIRED"
)
type ReserveInventoryCommand struct {
	BookingID, HotelID, RoomTypeID string
	CheckIn, CheckOut time.Time
	Quantity int
}
...
type AvailabilityRepository interface {
	ReserveInventory(context.Context, ReserveInventoryCommand) (Reservation, error)
	GetReservation(context.Context, string) (Reservation, error)
	ConfirmReservation(context.Context, string) (Reservation, error)
	ReleaseReservation(context.Context, string, string) (Reservation, error)
}
...
type CreatePaymentCommand struct {
	BookingID, IdempotencyKey, Currency, PaymentMethodRef string
	AmountMinor int64
}
type Payment struct {
	ID, BookingID string
...
type PaymentRepository interface {
	CreatePayment(context.Context, CreatePaymentCommand) (Payment, error)
