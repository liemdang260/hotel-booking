...
RETURNING event.id::text,event.aggregate_type,event.aggregate_id::text,event.event_type,event.event_version,
          event.payload,event.retry_count,event.created_at,$3::text`,
	...
	for rows.Next(){
		...
		if err:=rows.Scan(&event.ID,&event.AggregateType,&event.AggregateID,&event.EventType,&event.EventVersion,&event.Payload,&event.Attempt,&event.OccurredAt,&event.ClaimToken);err!=nil{return nil,fmt.Errorf("scan booking outbox: %w",err)}
