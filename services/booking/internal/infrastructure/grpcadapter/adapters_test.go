...
func (c *paymentClient) CreatePayment(ctx context.Context, _ *CreatePaymentRequest) (*PaymentResponse, error) {
	c.seen = ctx
	return c.response, c.err
}

func (c *paymentClient) GetPayment(ctx context.Context, _ *GetPaymentRequest) (*PaymentResponse, error) {
	c.seen = ctx
	return c.response, c.err
}
...
func TestPaymentAdapterPropagatesCallerContext(t *testing.T) {
	...
	ctx, cancel := context.WithTimeout(
		context.WithValue(context.Background(), key("trace"), "t-1"),
		time.Second,
	)
	defer cancel()

	client := &paymentClient{response: &PaymentResponse{
		PaymentID: "p1",
		BookingID: "b1",
		Status:    "SUCCEEDED",
	}}
	...
	if client.seen != ctx || client.seen.Value(key("trace")) != "t-1" {
		t.Fatal("caller context was replaced")
	}
	if _, ok := client.seen.Deadline(); !ok {
...
func TestStructuredReasonPreserved(t *testing.T) {
	...
	if !errors.Is(mapped, repository.ErrQuoteExpired) {
...
func TestReadUnavailableIsNotMutationUnknown(t *testing.T) {
	...
	if !errors.Is(err, repository.ErrDownstreamUnavailable) || errors.Is(err, repository.ErrOutcomeUnknown) {
		t.Fatalf("got %v", err)
	}
}
