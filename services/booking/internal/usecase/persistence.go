...
func NewPersistence(transactions domain.TransactionManager) *Persistence {
	return &Persistence{transactions: transactions}
}
...
func (p *Persistence) SaveStateWithEvent(
	ctx context.Context, booking *domain.Booking, saga *domain.BookingSaga, event *domain.OutboxEvent,
) error {
	return p.transactions.WithinTransaction(ctx, func(txctx context.Context, r domain.Repositories) error {
