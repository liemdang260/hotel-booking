...
func NewCreateBookingUsecase(
	p repository.PricingRepository,
	a repository.AvailabilityRepository,
	pay repository.PaymentRepository,
	s BookingSagaStore,
	guards ...*IdempotencyGuard,
) *CreateBookingUsecase {
	u := &CreateBookingUsecase{
		pricing:      p,
		availability: a,
		payment:      pay,
		state:        s,
...
func (u *CreateBookingUsecase) Execute(ctx context.Context, in CreateBookingInput) (CreateBookingOutput, error) {
	...
	if err != nil {
		return CreateBookingOutput{}, fmt.Errorf("get accepted quote: %w", err)
	}
	if !u.now().Before(quote.ExpiresAt) {
		return CreateBookingOutput{}, repository.ErrQuoteExpired
	}
	...
	if u.idempotency != nil {
		...
		record, guardErr := u.idempotency.BeginOrResume(ctx, in.IdempotencyKey, in.BookingID, identity)
		if guardErr != nil {
			return CreateBookingOutput{}, fmt.Errorf("claim create-booking idempotency: %w", guardErr)
		}
		if record.BookingID != in.BookingID {
			return CreateBookingOutput{BookingID: record.BookingID, Status: record.Status}, nil
		}
	}

	if err = u.state.CreatePriceAccepted(ctx, CreateBookingState{BookingID: in.BookingID, UserID: in.UserID, Quote: quote}); err != nil {
	...
	reservation, err := u.availability.ReserveInventory(ctx, repository.ReserveInventoryCommand{
		...
		Quantity:   quote.RoomQuantity,
	})
	if errors.Is(err, repository.ErrSoldOut) || errors.Is(err, repository.ErrInventoryNotConfigured) {
		if saveErr := u.state.MarkFailed(ctx, in.BookingID, "INVENTORY_UNAVAILABLE"); saveErr != nil {
			return CreateBookingOutput{}, saveErr
		}
		return CreateBookingOutput{BookingID: in.BookingID, Status: "FAILED"}, nil
	}
	if err != nil {
		return CreateBookingOutput{}, fmt.Errorf("reserve inventory: %w", err)
	}
	if err = u.state.MarkInventoryReserved(ctx, in.BookingID, reservation); err != nil {
		return CreateBookingOutput{}, err
	}

	paymentKey := in.IdempotencyKey + ":payment"
	if err = u.state.MarkPaymentRequested(ctx, in.BookingID, paymentKey); err != nil {
		return CreateBookingOutput{}, err
	}
	...
	if errors.Is(err, repository.ErrOutcomeUnknown) {
		if saveErr := u.state.MarkPaymentUnknown(ctx, in.BookingID, err); saveErr != nil {
			return CreateBookingOutput{}, saveErr
		}
	...
	if err != nil {
		return CreateBookingOutput{}, fmt.Errorf("request payment: %w", err)
	}
	...
	if payment.Status == repository.PaymentUnknown || payment.Status == repository.PaymentProcessing {
		if saveErr := u.state.MarkPaymentUnknown(ctx, in.BookingID, repository.ErrOutcomeUnknown); saveErr != nil {
			return CreateBookingOutput{}, saveErr
		}
		return CreateBookingOutput{BookingID: in.BookingID, Status: "PAYMENT_UNKNOWN"}, nil
	}
	if payment.Status != repository.PaymentSucceeded {
		return CreateBookingOutput{}, repository.ErrInvalidRemoteResponse
	}
	if err = u.state.MarkPaymentSucceeded(ctx, in.BookingID, payment); err != nil {
		return CreateBookingOutput{}, err
	}

	confirmed, err := u.availability.ConfirmReservation(ctx, reservation.ID)
	if errors.Is(err, repository.ErrOutcomeUnknown) {
		if saveErr := u.state.MarkConfirmationUnknown(ctx, in.BookingID, err); saveErr != nil {
			return CreateBookingOutput{}, saveErr
		}
	...
	if errors.Is(err, repository.ErrReservationExpired) {
		if saveErr := u.state.MarkCompensationRequired(ctx, in.BookingID, "REFUND_AFTER_RESERVATION_EXPIRED"); saveErr != nil {
			return CreateBookingOutput{}, saveErr
		}
		return CreateBookingOutput{BookingID: in.BookingID, Status: "COMPENSATING"}, nil
	}
	if err != nil {
		return CreateBookingOutput{}, fmt.Errorf("confirm reservation: %w", err)
	}
	if err = u.state.MarkConfirmed(ctx, in.BookingID, confirmed); err != nil {
		return CreateBookingOutput{}, err
	}
	return CreateBookingOutput{BookingID: in.BookingID, Status: "CONFIRMED"}, nil
}

func (u *CreateBookingUsecase) compensateConfirmedPaymentFailure(
	ctx context.Context,
	bookingID, reservationID, reason string,
) (CreateBookingOutput, error) {
	_, err := u.availability.ReleaseReservation(ctx, reservationID, reason)
	if err != nil {
		if saveErr := u.state.MarkCompensationRequired(ctx, bookingID, "RELEASE_HELD_INVENTORY"); saveErr != nil {
			return CreateBookingOutput{}, saveErr
		}
		return CreateBookingOutput{BookingID: bookingID, Status: "COMPENSATING"}, nil
	}
	if err = u.state.MarkPaymentFailedAndReleased(ctx, bookingID, reason); err != nil {
		return CreateBookingOutput{}, err
	}
